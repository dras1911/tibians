 ...
 *   house             15m  — same
 */
export const TTL_BY_ENDPOINT: Record<string, number> = {
  worlds: 24 * 60 * 60_000,
  world: 24 * 60 * 60_000,
...
export class LruCache {
  ...
  private readonly order = new Map<string, CacheEntry>();
  ...
  public set(key: string, value: unknown, ttlMs: number): void {
    if (ttlMs <= 0) return; // negative / zero TTL → cache disabled
    const entry: CacheEntry = { value, expiresAt: this.now() + ttlMs };
