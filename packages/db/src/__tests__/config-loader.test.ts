...
function createMockDb(rows: MockRow[]): {
  ...
  return { db: db as unknown as NodePgDatabase<typeof schema>, queries };
}

const ROW = (key: string, value: unknown): MockRow => ({
  key,
  value,
  description: 'test',
...
describe('createConfigLoader', () => {
  ...
  it('cache miss: pierwszy call czyta DB i zwraca wartość', async () => {
    ...
    const result = await loader.getConfig<number>('economy.tc_price_gold_threshold');
  ...
  it('TTL expiration: po 60s kolejny call ponownie czyta DB', async () => {
    ...
    const result = await loader.getConfig<number>('stamina.hours_per_day_full');
  ...
  it('wartość obiektowa (rekord) przechodzi walidację i wraca z cache', async () => {
    ...
    const result = await loader.getConfig<{ threshold: number; enabled: boolean }>(
      'test.object',
    );
