...
export const auctions = pgTable(
  'auctions',
  {
    ...
    pricePerLevel: numeric('price_per_level', { precision: 10, scale: 2 })
      .generatedAlwaysAs(
        (): SQL => sql`(${auctions.bid})::numeric / NULLIF(${auctions.level}, 0)`,
      ),

    /* ── PAYLOAD I META ───────────────────────────────────────── */
    // pełny scrape — future-proof na zmiany HTML Tibii (ubezpieczenie R1)
    rawJson: jsonb('raw_json')
      .$type<Record<string, unknown>>() // walidacja struktury po stronie aplikacji (Zod w scraperze)
      .notNull(),
    // GENERATED: full-text search na character_name
    searchVector: tsvector('search_vector').generatedAlwaysAs(
      (): SQL => sql`to_tsvector('simple', ${auctions.characterName})`,
    ),
