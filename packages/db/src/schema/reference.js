...
import { boolean, index, integer, pgEnum, pgTable, serial, smallint, smallserial, text, timestamp, } from 'drizzle-orm/pg-core';
/* ════════════════════════════════════════════════════════════════
...
export const imbuementTierEnum = pgEnum('imbuement_tier', [
    'basic',
    'powerful',
    'epic',
]);
...
export const worlds = pgTable('worlds', {
    ...
    pvpType: pvpTypeEnum('pvp_type').notNull(),
    battleye: battleyeEnum('battleye').notNull(),
    ...
    index('idx_worlds_active').on(table.isActive).where(sql `is_active = true`),
]);
/* ════════════════════════════════════════════════════════════════
 *  ITEMS
...
export const items = pgTable('items', {
    ...
    index('idx_items_name_trgm')
        .using('gin', sql `${table.name} gin_trgm_ops`),
    // partial index na rare — mały, szybki
    index('idx_items_rare').on(table.id).where(sql `is_rare = true`),
    index('idx_items_category').on(table.category),
    index('idx_items_store').on(table.id).where(sql `is_store_item = true`),
]);
/* ════════════════════════════════════════════════════════════════
 *  OUTFITS / MOUNTS
...
export const outfits = pgTable('outfits', {
    ...
    index('idx_outfits_rare').on(table.id).where(sql `is_rare = true`),
    index('idx_outfits_store').on(table.id).where(sql `is_store = true`),
]);
export const mounts = pgTable('mounts', {
    ...
    index('idx_mounts_rare').on(table.id).where(sql `is_rare = true`),
    index('idx_mounts_store').on(table.id).where(sql `is_store = true`),
]);
/* ════════════════════════════════════════════════════════════════
 *  IMBUEMENTS
 *  Słownik PL z Intibia (arch §2.3). 23 sloty w grze (max tier=3).
 * ════════════════════════════════════════════════════════════════ */
export const imbuements = pgTable('imbuements', {
    id: smallserial('id').primaryKey(),
...
export const quests = pgTable('quests', {
    ...
    index('idx_quests_notable').on(table.id).where(sql `is_notable = true`),
]);
/* ════════════════════════════════════════════════════════════════
 *  BOSSES
