/**
 * `/[locale]/reference/mounts` — stronicowana lista mountów (plan T61, arch
 * §7.2 + §18.1).
 *
 * Bliźniacza strona do `/reference/outfits` — ten sam wzorzec, inna tabela.
 *
 * **Fetch:** `listReferenceMounts()` z DB (T7 schema `mounts`, T32 scraper).
 * **ISR:** 24h (revalidate = 86_400).
 * **i18n:** `Reference.mounts.*` (PL + EN).
 */
...
import { listReferenceMounts } from "@/lib/server/reference-pages";
...
export async function generateMetadata({
  ...
  const t = await getTranslations({
    locale,
    namespace: "Reference.mounts",
  });

  const title = t("pageTitle");
  const description = t("pageDescription");
  const path = "/reference/mounts";
...
export default async function ReferenceMountsPage({
  params,
  searchParams,
}: {
  params: Promise<{ locale: string }>;
  ...
  const [t, tRef] = await Promise.all([
    getTranslations({ locale, namespace: "Reference.mounts" }),
    getTranslations({ locale, namespace: "Reference.index" }),
  ]);

  // Best-effort DB query.
  let result: Awaited<ReturnType<typeof listReferenceMounts>> | null = null;
  try {
    result = await listReferenceMounts({ page, pageSize: PAGE_SIZE });
  } catch (error) {
    // eslint-disable-next-line no-console
    console.error("[reference/mounts] DB query failed:", error);
    result = null;
  }
  ...
  const breadcrumbItems: BreadcrumbItem[] = [
    ...
    { label: t("pageTitle"), href: "/reference/mounts" },
  ];
  ...
  const prevHref = page > 1 ? `/reference/mounts?page=${(page - 1).toString()}` : null;
  const nextHref = page < totalPages ? `/reference/mounts?page=${(page + 1).toString()}` : null;
...
// ───────────────────────────────────────────────────────────────────────
// MountCard — karta mounta (image + badges + PL/EN name).
// ───────────────────────────────────────────────────────────────────────
